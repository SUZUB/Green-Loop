    ...
    <div className="min-h-screen pb-20">
      ...
      <nav className="sticky top-0 z-50 backdrop-blur-md border-b" style={{ background: "rgba(248,250,249,0.95)", borderColor: "#D1FAE5" }}>
        <div className="container mx-auto flex items-center justify-between h-14 px-4">
          ...
          <button className="relative inline-flex items-center gap-2 rounded-full border border-[#D1FAE5] bg-white/95 px-3 py-2 text-sm text-[#1E293B]">
            <Bell className="h-4 w-4" /> Notifications
            {notificationCount > 0 && (
              <span className="absolute -right-2 -top-2 inline-flex h-5 min-w-[1.25rem] items-center justify-center rounded-full bg-[#10B981] text-[11px] text-[#1E293B] px-1.5">
                {notificationCount}
              </span>
            )}
          </button>
      ...
      <div className="container mx-auto max-w-7xl px-4 py-6">
        <div className="grid gap-6 xl:grid-cols-[1.35fr_0.9fr]">
          <div className="space-y-6">
            <div className="rounded-[32px] bg-white/10 border border-[#D1FAE5] p-6 shadow-2xl backdrop-blur-xl">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className="text-xs uppercase tracking-[0.3em] text-emerald-200/75">Verified Suppliers</p>
                  <h1 className="mt-2 text-2xl font-semibold text-[#1E293B]">Supplier directory</h1>
                </div>
                <div className="flex items-center gap-3 rounded-full bg-white/95 px-4 py-3 text-sm text-[#14532D]">
                  <Shield className="h-4 w-4 text-[#065F46]" /> Verified partners
                </div>
              </div>
              <div className="mt-5 flex items-center gap-3 rounded-3xl border border-[#D1FAE5] bg-[#F8FAF9]/70 px-4 py-3">
                <Input
                  value={search}
                  onChange={(event) => setSearch(event.target.value)}
                  placeholder="Search suppliers, materials, city"
                  className="bg-transparent text-[#1E293B] placeholder:text-[#475569]"
                />
              </div>
            </div>
                ...
                <motion.div key={s.id} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
                  ...
                  >
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <div className="flex items-center gap-2">
                          <h3 className="font-semibold text-sm text-[#1E293B]">{s.name}</h3>
                          {s.verified && (
                            <span className="flex items-center gap-0.5 text-xs text-[#14532D]">
                              <Shield className="h-3 w-3" /> Verified
                            </span>
                          )}
                        </div>
                        <p className="text-xs text-[#475569] mt-0.5">{s.location} · {s.types.join(", ")}</p>
                      </div>
                      <div className="flex items-center gap-1">
                        <Star className="h-3.5 w-3.5 text-yellow-500 fill-yellow-500" />
                        <span className="text-sm font-semibold text-[#1E293B]">{s.rating}</span>
                      </div>
                    </div>
            ...
            <div className="rounded-[32px] bg-white/95 border border-[#D1FAE5] p-6 shadow-2xl">
              <div className="flex items-center justify-between gap-4 mb-4">
                <div>
                  <p className="text-xs uppercase tracking-[0.3em] text-[#475569]">Pending requests</p>
                  <h2 className="text-xl font-semibold text-[#1E293B]">Application queue</h2>
                </div>
                <span className="rounded-full bg-white/10 px-3 py-2 text-xs text-[#14532D]">{pendingApplications.length} open</span>
              </div>
              <div className="space-y-3">
                {pendingApplications.map((app, i) => (
                  <Card key={`${app.supplier}-${i}`} className="p-3 bg-white/95 border border-[#D1FAE5]">
                    <div className="flex items-center justify-between mb-2">
                      <div>
                        <p className="font-semibold text-sm text-[#1E293B]">{app.supplier}</p>
                        <p className="text-xs text-[#475569]">{app.type} · {app.qty}</p>
                      </div>
                      <span className="text-[11px] uppercase tracking-[0.22em] text-[#475569]">{app.applied}</span>
                    </div>
        ...
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="font-display flex items-center gap-2">
              ...
              {selectedSupplier?.verified && <Shield className="h-4 w-4 text-[#14532D]" />}
            </DialogTitle>
          </DialogHeader>
          {selectedSupplier && (
            <div className="space-y-4 pt-2">
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-sm text-[#475569]">({selectedSupplier.reviews} reviews)</span>
                </div>
              </div>

              <div className="grid grid-cols-3 gap-3">
                <Card className="p-3 text-center">
                  <Package className="h-4 w-4 mx-auto mb-1 text-[#065F46]" />
                  <p className="text-lg font-bold">{selectedSupplier.orders}</p>
                  <p className="text-xs text-[#475569]">Orders</p>
                </Card>
                <Card className="p-3 text-center">
                  <Clock className="h-4 w-4 mx-auto mb-1 text-[#14532D]" />
                  <p className="text-lg font-bold">{selectedSupplier.onTime}%</p>
                  <p className="text-xs text-[#475569]">On-time</p>
                </Card>
                <Card className="p-3 text-center">
                  <TrendingUp className="h-4 w-4 mx-auto mb-1 text-[#14532D]" />
                  <p className="text-lg font-bold">{selectedSupplier.totalQty}</p>
                  <p className="text-xs text-[#475569]">Supplied</p>
                </Card>
              </div>
              ...
              <div>
                ...
                <p className="text-sm text-[#475569]">{selectedSupplier.location}</p>
              </div>

              <div className="flex gap-3">
                <Button className="flex-1 bg-[#10B981] text-white hover:bg-[#10B981]/90 gap-1">
