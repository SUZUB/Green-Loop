...
const statusConfig = {
  paid: { color: "bg-[#DCFCE7] text-[#14532D]", icon: CheckCircle2, label: "Paid" },
  pending: { color: "bg-warning/10 text-warning", icon: Clock, label: "Pending" },
  overdue: { color: "bg-[rgba(220,38,38,0.1)] text-red-400", icon: AlertCircle, label: "Overdue" },
};
      ...
      <nav className="sticky top-0 z-50 backdrop-blur-md border-b" style={{ background: "rgba(248,250,249,0.95)", borderColor: "#D1FAE5" }}>
        <div className="container mx-auto flex items-center justify-between h-14 px-4">
          ...
          <Button
            ...
            className={bulkMode ? "bg-[#10B981] text-white" : ""}
          >
      ...
      <div className="container mx-auto px-4 py-6 max-w-lg">
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          ...
          <Card className="p-3 text-center">
            ...
            <p className="text-lg font-display font-bold text-[#14532D]">₹{paidTotal.toLocaleString()}</p>
          </Card>
          <Card className="p-3 text-center">
            <p className="text-xs text-[#475569]">Total GST</p>
        ...
        <div className="flex gap-2 mb-4">
          {(["all", "pending", "paid"] as const).map((t) => (
            <Button
              ...
              size="sm"
              onClick={() => setTab(t)}
              className={tab === t ? "bg-[#10B981] text-white" : ""}
            >
                    ...
                    <div className="flex-1">
                      <div className="flex items-start justify-between mb-1">
                        <div>
                          ...
                          <p className="text-xs text-[#475569]">{inv.supplier}</p>
                        </div>
                      ...
                      <div className="flex gap-2 mt-3">
                        ...
                        {inv.status !== "paid" && (
                          <Button size="sm" className="h-7 gap-1 text-xs bg-[#10B981] text-white hover:bg-[#10B981]/90 ml-auto"
        ...
        {bulkMode && selectedIds.length > 0 && (
          <motion.div
            ...
            className="fixed bottom-16 left-0 right-0 bg-card/95 backdrop-blur-md border-t border-[#D1FAE5] p-4 z-40"
          >
        ...
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="font-display flex items-center gap-2">
              <Receipt className="h-5 w-5 text-[#065F46]" /> {selectedInvoice?.id}
            </DialogTitle>
          </DialogHeader>
          {selectedInvoice && (
            <div className="space-y-5 pt-2">
              ...
              <div className="flex justify-between text-sm">
                <div>
                  ...
                  <p className="text-xs text-[#475569]">Date: {selectedInvoice.date}</p>
                </div>
                <div className="text-right">
                  <p className="font-semibold">To: Greenway Industries</p>
                  <p className="text-xs text-[#475569]">GSTIN: 27AADCG1234F1ZH</p>
                </div>
              </div>

              {/* Items table */}
              <div className="border border-[#D1FAE5] rounded-lg overflow-hidden">
                <div className="grid grid-cols-12 gap-1 p-3 bg-white/50 text-xs font-semibold">
                ...
                {selectedInvoice.items.map((item, i) => (
                  <div key={i} className="grid grid-cols-12 gap-1 p-3 text-xs border-t border-[#D1FAE5]">
              ...
              <Card className="p-4 space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-[#475569]">CGST (9%)</span>
                  <span>₹{(selectedInvoice.gst / 2).toLocaleString()}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-[#475569]">SGST (9%)</span>
                  <span>₹{(selectedInvoice.gst / 2).toLocaleString()}</span>
                </div>
                <div className="flex justify-between text-sm font-bold border-t border-[#D1FAE5] pt-2 mt-2">
              ...
              <div className="flex gap-3">
                ...
                {selectedInvoice.status !== "paid" && (
                  <Button className="flex-1 gap-1 bg-[#10B981] text-white hover:bg-[#10B981]/90">
