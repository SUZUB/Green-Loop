...
const typeBreakdown = [
  { type: "PET", qty: "5,200 kg", pct: 42, cost: "₹78,000", color: "bg-[#10B981]" },
  { type: "HDPE", qty: "3,100 kg", pct: 25, cost: "₹37,200", color: "bg-[#10B981]" },
  { type: "PP", qty: "2,400 kg", pct: 19, cost: "₹43,200", color: "bg-[#10B981]" },
    ...
    <div className="min-h-screen pb-20">
      ...
      <div className="container mx-auto px-4 py-6 max-w-lg">
        ...
        <h2 className="text-lg font-display font-bold mb-3 flex items-center gap-2">
          <BarChart3 className="h-5 w-5 text-[#065F46]" /> Purchase Summary
        </h2>
            ...
            <motion.div key={p.label} initial={{ opacity: 0, x: -15 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}>
              <Card className="p-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-[#475569]">{p.amount}</p>
                </div>
                {p.change && (
                  <span className={`text-sm font-semibold flex items-center gap-1 ${p.up ? "text-[#14532D]" : "text-red-400"}`}>
        ...
        <h2 className="text-lg font-display font-bold mb-3 flex items-center gap-2">
          <PieChart className="h-5 w-5 text-[#065F46]" /> Plastic Type Breakdown
        </h2>
        <div className="space-y-3 mb-8">
            ...
            <motion.div key={t.type} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 + i * 0.05 }}>
              <Card className="p-4">
                ...
                <div className="flex items-center justify-between mt-1">
                  <span className="text-xs text-[#475569]">{t.pct}% of total</span>
                  <span className="text-xs text-[#475569]">{t.cost}</span>
                </div>
              </Card>
            </motion.div>
          ))}
        ...
        {/* Price Trends */}
        <h2 className="text-lg font-display font-bold mb-3 flex items-center gap-2">
          <IndianRupee className="h-5 w-5 text-[#065F46]" /> Market Price Trends
        </h2>
          ...
          {priceTrends.map((p, i) => (
            <motion.div key={p.type} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 + i * 0.05 }}>
              <Card className="p-4">
                ...
                <div className="flex items-center justify-between mt-1">
                  <span className="text-xs text-[#475569]">Avg: {p.avg}</span>
                  <span className={`text-xs font-semibold ${p.up ? "text-red-400" : "text-[#14532D]"}`}>{p.trend}</span>
                </div>
              </Card>
            </motion.div>
          ))}
        ...
        <h2 className="text-lg font-display font-bold mb-3 flex items-center gap-2">
          <Package className="h-5 w-5 text-[#065F46]" /> Supplier Performance
        </h2>
            ...
            <motion.div key={s.name} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 + i * 0.05 }}>
              <Card className="p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-xs font-bold bg-[#DCFCE7] text-[#14532D] px-2 py-0.5 rounded-full">{s.score}</span>
                </div>
                <div className="grid grid-cols-3 gap-2 text-xs text-[#475569]">
                  <div>
                    <p className="font-medium text-[#1E293B]">{s.onTime}%</p>
                    <p>On-time</p>
                  </div>
                  <div>
                    <p className="font-medium text-[#1E293B]">{s.quality}%</p>
                    <p>Quality</p>
                  </div>
                  <div>
                    <p className="font-medium text-[#1E293B]">{s.price}</p>
                    <p>Avg price</p>
                  </div>
                </div>
              </Card>
