    ...
    <div className="min-h-screen bg-background/40 pb-20">
      ...
      <nav className="sticky top-0 z-50 bg-background/80 backdrop-blur-md border-b border-border">
        <div className="container mx-auto flex items-center justify-between h-14 px-4">
          ...
          <Button size="sm" onClick={() => setShowCreate(true)} className="gap-1 bg-earth text-earth-foreground hover:bg-earth/90">
              ...
              <Card className="p-4">
                <div className="flex items-start justify-between mb-2">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{listing.materialType} · {listing.quantity} · {listing.requestedBy}</p>
                  </div>
                  <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                    listing.status === "Active" ? "bg-primary/10 text-primary" : "bg-warning/10 text-warning"
                  }`}>
                    {listing.status}
                  </span>
                </div>
        ...
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          ...
          <div className="space-y-4 pt-2">
            ...
            <div className="flex gap-3 pt-2">
              <Button className="flex-1 bg-earth text-earth-foreground hover:bg-earth/90">Post Listing</Button>
