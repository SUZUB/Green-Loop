      ...
      <div className="h-screen bg-background/40 flex flex-col overflow-hidden">
        ...
        <nav className="sticky top-0 z-50 bg-background/80 backdrop-blur-md border-b border-border">
          <div className="container mx-auto flex items-center gap-3 h-14 px-4 max-w-lg">
            ...
            <div className="w-9 h-9 rounded-full bg-earth/20 flex items-center justify-center text-sm font-bold text-earth">
              {selectedChat.avatar}
            </div>
            <div className="flex-1">
              <div className="flex items-center gap-1.5">
                <span className="font-semibold text-sm">{selectedChat.name}</span>
                {selectedChat.verified && <Shield className="h-3 w-3 text-primary" />}
              </div>
              ...
              >
                <div
                  className={`max-w-[80%] px-4 py-2.5 rounded-2xl text-sm ${
                    msg.sender === "buyer"
                      ? "bg-earth text-earth-foreground rounded-br-md"
                      : "bg-card border border-border rounded-bl-md"
                  }`}
                >
                  <p>{msg.text}</p>
                  <p className={`text-[10px] mt-1 ${
                    msg.sender === "buyer" ? "text-earth-foreground/60" : "text-muted-foreground"
                  }`}>
                    {msg.time}
                  </p>
                </div>
        ...
        <div className="sticky bottom-0 bg-background/95 backdrop-blur-md border-t border-border p-3">
          <div className="max-w-lg mx-auto flex items-center gap-2">
            ...
            <Button
              ...
              className="bg-earth text-earth-foreground hover:bg-earth/90 p-2 shrink-0"
            >
      ...
      <nav className="sticky top-0 z-50 bg-background/80 backdrop-blur-md border-b border-border">
        <div className="container mx-auto flex items-center justify-between h-14 px-4">
          <div className="flex items-center gap-2">
            <MessageSquare className="h-6 w-6 text-earth" />
            ...
            >
              ...
              >
                <div className="relative">
                  <div className="w-12 h-12 rounded-full bg-earth/15 flex items-center justify-center text-sm font-bold text-earth">
                    {conv.avatar}
                  </div>
                  {conv.online && (
                    <Circle className="absolute -bottom-0.5 -right-0.5 h-3.5 w-3.5 fill-primary text-primary stroke-background stroke-2" />
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1.5">
                      ...
                      {conv.verified && <Shield className="h-3 w-3 text-primary" />}
                    </div>
                  ...
                  <div className="flex items-center justify-between mt-0.5">
                    ...
                    {conv.unread > 0 && (
                      <span className="w-5 h-5 rounded-full bg-earth text-earth-foreground text-[10px] font-bold flex items-center justify-center shrink-0">
                        {conv.unread}
                      </span>
                    )}
                  </div>
