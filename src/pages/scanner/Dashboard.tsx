        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_420px]">
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2 mb-1">
              <ClipboardList className="h-4 w-4 text-[#475569]" />
            ...
            {loading && pickups.length === 0 ? (
              <div className="flex justify-center py-12">
                <Loader2 className="h-7 w-7 animate-spin text-[#14532D]" />
              </div>
            ) : pickups.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                <Package className="h-10 w-10 text-[#475569] opacity-40 mb-3" />
                <p className="text-[#475569] text-sm">No items pending scan.</p>
                <p className="text-xs text-[#475569] mt-1">Items appear here when recyclers submit them.</p>
              </div>
            ) : (
                  ...
                  >
                    <Card
                      className={`p-4 cursor-pointer transition-all border-2 ${
                        selected?.id === p.id
                          ...
                          : "border-transparent hover:border-[#D1FAE5]"
                      }`}
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <Badge variant="outline" className="text-[#14532D] border-amber-300 shrink-0">
                          PENDING SCAN
                        </Badge>
                      </div>
                      ...
                      {p.notes && (
                        <p className="text-xs text-[#475569] mt-2 bg-white/50 rounded px-2 py-1">{p.notes}</p>
                      )}
                    </Card>
                  </motion.div>
                ))}
              ...
              {selected ? (
                <motion.div key={selected.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }}>
                  <Card className="p-5 space-y-4 sticky top-6">
                    <div className="flex items-center gap-2">
                      <ScanLine className="h-5 w-5 text-[#14532D]" />
                      <h3 className="font-display font-bold">Validate Item</h3>
                    </div>

                    <div className="rounded-lg bg-white/50 p-3 text-sm space-y-1">
                      <p><span className="text-[#475569]">Recycler:</span> <span className="font-medium">{selected.recycler_name}</span></p>
                      <p><span className="text-[#475569]">Declared weight:</span> <span className="font-medium">{selected.weight_kg} kg</span></p>
                    </div>

                    <div className="space-y-3">
                      <div>
                        <Label>Actual Weight (kg) <span className="text-red-400">*</span></Label>
                        <Input
                          type="number"
                          step="0.1"
                          min="0.1"
                      ...
                      <div>
                        <Label>Plastic Type <span className="text-red-400">*</span></Label>
                      ...
                      <div>
                        <Label>Quality Grade <span className="text-red-400">*</span></Label>
                    ...
                    {form.weight_kg && parseFloat(form.weight_kg) > 0 && (
                      <div className="rounded-lg bg-[#F0FDF4] border border-[#D1FAE5] p-3 text-sm">
                        <p className="text-[#475569]">Payment credits generated:</p>
                        <p className="text-xl font-display font-bold text-[#14532D]">
                          {Math.round(parseFloat(form.weight_kg) * 100)} pts
                        </p>
                        <p className="text-xs text-[#475569] mt-0.5">Locked until picker completes delivery</p>
                      </div>
                    )}
                ...
                <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  <Card className="p-8 text-center border-dashed">
                    <ScanLine className="h-10 w-10 text-[#475569] opacity-40 mx-auto mb-3" />
                    <p className="text-[#475569] text-sm">Select an item from the queue to validate it.</p>
                  </Card>
                </motion.div>
              )}
            </AnimatePresence>
