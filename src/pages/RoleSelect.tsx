...
const RoleSelect = () => {
  ...
  return (
    <div className="relative min-h-screen bg-transparent flex flex-col isolate">
      ...
      <div className="relative z-10 flex-1 flex items-center justify-center px-4">
        <div className="max-w-4xl w-full">
          ...
          >
            ...
            <p className="text-[#475569]">Join our plastic economy as a...</p>
          </motion.div>
          <div className="grid md:grid-cols-3 gap-6">
            {roles.map((role, i) => (
              <motion.button
                ...
                whileHover={{ y: -6 }}
                className="p-8 rounded-2xl bg-card shadow-card hover:shadow-elevated transition-all duration-300 text-left group border border-[#D1FAE5] hover:border-[#D1FAE5]"
              >
                ...
                <h2 className="text-xl font-display font-semibold mb-2 group-hover:text-[#14532D] transition-colors">
                  {role.title}
                </h2>
                <p className="text-[#475569] text-sm mb-5">{role.description}</p>
                <span className="inline-flex items-center gap-1 text-sm font-medium text-[#14532D] opacity-0 group-hover:opacity-100 transition-opacity">
                  Continue →
                </span>
              </motion.button>
            ))}
