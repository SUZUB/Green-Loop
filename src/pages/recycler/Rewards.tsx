    ...
    <div className="min-h-screen bg-background/40">
      ...
      <div className="container mx-auto px-4 py-6 max-w-lg">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
          ...
          <p className="text-muted-foreground text-sm">Your verified carbon credit portfolio</p>
        </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
            <Card className="p-5 border-l-4 border-l-emerald">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs text-muted-foreground uppercase tracking-wide">Verified Carbon Credits</p>
                  <p className="text-3xl font-display font-bold text-emerald">{carbonCredits.toLocaleString("en-IN")}</p>
                  <p className="text-xs text-muted-foreground mt-1">From {stats.totalRecycledKg} kg recycled • 5 credits/kg</p>
                </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
              <Card className="p-4 border-l-4 border-l-gold">
                ...
                <p className="text-[10px] text-muted-foreground">@ ₹100/credit</p>
              </Card>
            </motion.div>
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
              <Card className="p-4 border-l-4 border-l-primary">
                <p className="text-xs text-muted-foreground uppercase tracking-wide">Redeemable Cash</p>
                <p className="text-xl font-display font-bold">₹{cashBalanceINR.toLocaleString("en-IN")}</p>
                <p className="text-[10px] text-muted-foreground">INR Balance</p>
              </Card>
            </motion.div>
          </div>
        </div>
        ...
        <div className="space-y-2">
          {ledgerLoading ? (
            ...
            <p className="text-center text-muted-foreground py-8 text-sm">No transactions yet. Start recycling to earn carbon credits!</p>
          ) : (
            ledger.map((entry, i) => (
