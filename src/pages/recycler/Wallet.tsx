...
const plans = [
  { name: "Silver", cost: 500, color: "bg-[#F0FDF4]", perks: ["₹50 subscription discount", "Basic offers access"] },
  { name: "Gold", cost: 1500, color: "bg-[#10B981]", perks: ["Premium offers", "Priority pickups", "Partner discounts"] },
      ...
      <div className="container mx-auto px-4 py-6 max-w-lg">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <Card className="bg-hero-gradient p-6 rounded-2xl mb-6">
            ...
            <div className="text-4xl font-display font-bold text-white">{userBalance} pts</div>
            <p className="text-white/60 text-xs mt-1">≈ ₹{Math.floor(userBalance / 10)}</p>
          </Card>
        </motion.div>
        ...
        <Tabs value={tab} onValueChange={setTab}>
          ...
          <TabsContent value="overview" className="space-y-2">
            ...
            {fullTransactionList.length === 0 ? (
              <p className="text-center text-[#475569] py-8 text-sm">No transactions yet.</p>
            ) : (
                ...
                <motion.div key={tx.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }}>
                  <Card className="p-3 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full flex items-center justify-center bg-[#F0FDF4] text-[#14532D]">
                        {tx.amount >= 0 ? <ArrowUpRight className="h-4 w-4" /> : <ArrowDownLeft className="h-4 w-4" />}
                      </div>
                      <div>
                        <p className="text-sm font-medium">{tx.description}</p>
                        <p className="text-[10px] text-[#475569]">{new Date(tx.createdAt).toLocaleDateString("en-IN", { month: "short", day: "numeric" })}</p>
                      </div>
                    </div>
                    <span className={`font-display font-bold text-sm ${tx.amount >= 0 ? "text-[#14532D]" : "text-red-400"}`}>
            ...
            <Card className="p-5 space-y-4">
              <div>
                ...
                {cashoutAmount && parseInt(cashoutAmount) >= 1000 && (
                  <p className="text-xs text-[#14532D] mt-1">You'll receive ₹{parseInt(cashoutAmount) / 10}</p>
                )}
              </div>
              ...
              <p className="text-xs text-[#475569]">Processing time: 24-48 hours</p>
                  ...
                  <Card className="p-5">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-[#475569]">{plan.cost} pts/month</p>
                      </div>
                    </div>
                    <ul className="space-y-1 mb-3">
                      {plan.perks.map((p) => (
                        <li key={p} className="text-xs text-[#475569] flex items-center gap-1.5"><span className="text-[#14532D]">✓</span> {p}</li>
                      ))}
                    </ul>
                  ...
                  <Card className="p-5">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-[#475569] mt-0.5">{ngo.mission}</p>
