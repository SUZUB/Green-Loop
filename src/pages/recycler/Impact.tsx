...
const Impact = () => {
  ...
  const impactMetrics = [
    { icon: Leaf, label: "Plastic Recycled", value: `${stats.totalRecycledKg} kg`, target: "100 kg", progress: Math.min(stats.totalRecycledKg / 100 * 100, 100), color: "text-primary" },
    { icon: Droplets, label: "CO₂ Saved", value: `${stats.co2Saved.toFixed(1)} kg`, target: "1,000 kg", progress: Math.min(stats.co2Saved / 1000 * 100, 100), color: "text-ocean" },
    ...
    { icon: Fuel, label: "Oil Barrels Saved", value: (stats.totalRecycledKg * 0.064).toFixed(1), target: "10", progress: Math.min((stats.totalRecycledKg * 0.064) / 10 * 100, 100), color: "text-earth" },
    { icon: Droplets, label: "Water Saved", value: `${Math.round(stats.totalRecycledKg * 300).toLocaleString()} L`, target: "100,000 L", progress: Math.min((stats.totalRecycledKg * 300) / 100000 * 100, 100), color: "text-ocean" },
  ];
      ...
      <div className="container mx-auto px-4 py-6 max-w-lg">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <Card className="bg-hero-gradient p-6 rounded-2xl mb-6 text-center">
            <Leaf className="h-10 w-10 text-primary-foreground mx-auto mb-3" />
            <div className="text-4xl font-display font-bold text-primary-foreground">{stats.totalRecycledKg} kg</div>
            <p className="text-primary-foreground/70 text-sm mt-1">Total Plastic Recycled</p>
          ...
          {impactMetrics.map((metric, i) => (
            <motion.div key={metric.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}>
              <Card className="p-4 h-full">
                ...
                <p className="text-[10px] text-muted-foreground mt-1">Goal: {metric.target}</p>
              </Card>
            </motion.div>
          ))}
        </div>
            ...
            {milestones.map((m) => (
              <div key={m.weight} className="flex flex-col items-center gap-1">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                  m.reached ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"
                }`}>
                  {m.reached ? <Award className="h-4 w-4" /> : <span className="text-xs">{m.label}</span>}
                </div>
                {m.reached && <span className="text-[10px] text-primary font-medium">✓</span>}
              </div>
            ))}
          </div>
