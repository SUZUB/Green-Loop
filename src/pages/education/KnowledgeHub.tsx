...
const plasticGuide = [
  { code: 1, symbol: "♻️ 1", name: "PET/PETE", examples: "Water bottles, food containers", recyclable: true, color: "bg-[#10B981]" },
  { code: 2, symbol: "♻️ 2", name: "HDPE", examples: "Milk jugs, detergent bottles, pipes", recyclable: true, color: "bg-[#10B981]" },
  ...
  { code: 5, symbol: "♻️ 5", name: "PP", examples: "Yogurt cups, bottle caps, straws", recyclable: true, color: "bg-[#10B981]" },
  { code: 6, symbol: "♻️ 6", name: "PS", examples: "Styrofoam, disposable cups, packaging", recyclable: false, color: "bg-destructive" },
  { code: 7, symbol: "♻️ 7", name: "Other", examples: "Mixed plastics, polycarbonate", recyclable: false, color: "bg-[#F0FDF4]" },
];
      ...
      <div className="container mx-auto px-4 py-6 max-w-lg">
        <Tabs value={tab} onValueChange={setTab}>
                ...
                <motion.div key={article.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
                  <Card className="p-4 hover:shadow-soft transition-shadow cursor-pointer">
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <span className="text-[10px] text-[#475569]">{article.readTime}</span>
                    </div>
                    <h3 className="font-semibold text-sm mb-1">{article.title}</h3>
                    <p className="text-xs text-[#475569]">{article.excerpt}</p>
                  </Card>
                </motion.div>
              ))}
            </div>
          ...
          <TabsContent value="guide">
            <p className="text-sm text-[#475569] mb-4">Look for the recycling symbol on your plastic items to identify the type.</p>
            <div className="space-y-3">
              {plasticGuide.map((item, i) => (
                <motion.div key={item.code} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}>
                  <Card className="p-4 flex items-start gap-3">
                    <div className={`w-12 h-12 rounded-lg ${item.color} flex items-center justify-center text-white font-bold text-sm shrink-0`}>
                      #{item.code}
                    </div>
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-[#475569] mt-0.5">{item.examples}</p>
                    </div>
                  </Card>
                </motion.div>
              ))}
          ...
          <TabsContent value="newsletter">
            <Card className="p-6 text-center">
              <Mail className="h-10 w-10 text-[#14532D] mx-auto mb-3" />
              ...
              <p className="text-[10px] text-[#475569] mt-3">Unsubscribe anytime. We respect your inbox.</p>
            </Card>
          </TabsContent>
        </Tabs>
      </div>
