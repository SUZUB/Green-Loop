...
function PostModal({
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-50 bg-white/95 backdrop-blur-sm flex items-end sm:items-center justify-center p-4"
      onClick={onClose}
    >
      <motion.div
        ...
        className="bg-card border border-[#D1FAE5] rounded-2xl w-full max-w-lg max-h-[85vh] flex flex-col shadow-elevated"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-start justify-between gap-3 p-5 border-b border-[#D1FAE5]">
          <div className="flex-1">
            <p className="text-xs text-[#475569] mb-1">{post.author} · {post.time}</p>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          <p className="text-sm text-[#475569] leading-relaxed">{post.body}</p>
            ...
            <div className="space-y-3 pt-2">
              ...
              {comments.map((c, i) => (
                <div key={i} className="rounded-xl bg-white/50 px-4 py-3">
                  <p className="text-xs font-semibold mb-1">{c.author} <span className="font-normal text-[#475569]">· {c.time}</span></p>
        ...
        {/* Comment input */}
        <div className="p-4 border-t border-[#D1FAE5] flex gap-2">
          <Input
  ...
  return (
    ...
    >
      <motion.div
        ...
        exit={{ y: 60, opacity: 0 }}
        transition={{ type: "spring", stiffness: 300, damping: 30 }}
        className="bg-card border border-[#D1FAE5] rounded-2xl w-full max-w-lg shadow-elevated"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between p-5 border-b border-[#D1FAE5]">
                    ...
                    <Card className={`p-5 ${ch.isHeatmap ? "border-l-4 border-l-destructive" : ""}`}>
                      <div className="flex items-start justify-between mb-2">
                        <div className="flex items-center gap-2">
                          {ch.isHeatmap
                            ...
                            : <Target className="h-5 w-5 text-[#14532D]" />}
                      ...
                      {ch.isHeatmap && ch.location && (
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <Badge className={`text-[10px] ${ch.density === "Critical" ? "bg-destructive" : "bg-orange-500"} text-[#1E293B] border-0`}>
                            {ch.density} Density
                          </Badge>
                        </div>
                      )}

                      <p className="text-xs text-[#475569] mb-3">{ch.desc}</p>
                      ...
                      <div className="flex items-center justify-between pt-3 border-t border-[#D1FAE5] gap-2 flex-wrap">
                ...
                <motion.div key={earner.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}>
                  <Card className="p-4 flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm ${
                      ...
                      earner.rank === 2 ? "bg-muted text-[#475569]" :
                      earner.rank === 3 ? "bg-[#10B981] text-white" :
                      "bg-[#F0FDF4] text-[#1E293B]"
                    }`}>
                      {earner.rank <= 3 ? ["🥇", "🥈", "🥉"][earner.rank - 1] : earner.avatar}
                    </div>
                    <div className="flex-1">
                      <p className="font-semibold text-sm">{earner.name}</p>
                      <p className="text-xs text-[#475569]">{earner.kg} kg recycled</p>
                    </div>
                    <div className="text-right">
                      <p className="font-display font-bold text-emerald">{earner.credits.toLocaleString("en-IN")}</p>
                      <p className="text-[10px] text-[#475569]">Carbon Credits</p>
                    </div>
                  </Card>
                </motion.div>
              ))}
