...
import {
  MapPin, Package, History, User, ScanLine, Sprout, Menu,
} from "lucide-react";
import { QRScannerModal } from "@/components/picker/QRScannerModal";
import { useToast } from "@/hooks/use-toast";

const pickerNav = [
  { id: "dashboard",  label: "Dashboard",  icon: MapPin,    path: "/picker/dashboard" },
...
function PickerSidebar({ collapsed }: { collapsed: boolean }) {
  ...
  return (
    <>
      ...
      >
          ...
          <div className={`flex items-center ${collapsed ? "justify-center" : "gap-3"}`}>
            ...
            {!collapsed && (
              <div>
                ...
                <p className="text-[10px] text-[#475569] font-medium uppercase tracking-widest">Picker</p>
              </div>
            )}
          </div>
        </div>
        ...
        {/* Scan QR CTA */}
        <div className={`${collapsed ? "px-2 py-3" : "px-4 py-3"} border-b border-[#D1FAE5]`}>
          <button
            onClick={() => setScannerOpen(true)}
            title={collapsed ? "Scan QR" : undefined}
            className={`w-full flex items-center ${collapsed ? "justify-center" : "gap-3"} px-3 py-2.5 rounded-xl bg-[#10B981] hover:bg-[#059669] text-white transition-colors font-semibold text-sm`}
          >
            <ScanLine className="h-4 w-4 shrink-0" />
            {!collapsed && "Scan Recycler QR"}
          </button>
        </div>
      ...
      <QRScannerModal
        ...
        onSuccess={(result) => {
          ...
          toast({
            ...
            description: `${result.weight_kg} kg collected · ${result.points} credits added.`,
          });
        }}
      />
    </>
...
export function PickerLayout() {
  const [collapsed, setCollapsed] = useState(() => {
    try { return window.localStorage.getItem("picker_sidebar_collapsed") === "true"; }
    catch { return false; }
  });

  useEffect(() => {
    try { window.localStorage.setItem("picker_sidebar_collapsed", String(collapsed)); }
    catch { /* ignore */ }
  }, [collapsed]);
  ...
  return (
    <div className="flex min-h-screen w-screen" style={{ background: "#F9FAFB" }}>
      ...
      <div className={`sticky top-0 h-screen shrink-0 z-50 transition-[width] duration-200 ${collapsed ? "w-20" : "w-64"}`}>
        <PickerSidebar collapsed={collapsed} />
      </div>
