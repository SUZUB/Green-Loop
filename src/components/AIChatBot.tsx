...
export function AIChatBot() {
  ...
  const quickPromptButtons = suggestedPrompts.map((prompt) => (
    <button
      ...
      className="rounded-full border border-[#D1FAE5] bg-white/95 px-3 py-1.5 text-xs text-[#475569] hover:bg-[#F0FDF4] transition"
    >
      {prompt}
    </button>
  ));
    ...
    <>
      ...
      >
        <Button
          ...
          className="w-14 h-14 rounded-full bg-gradient-to-br from-[#4caf7d] to-blue-500 text-[#1E293B] shadow-[0_20px_50px_rgba(59,130,246,0.35)] hover:shadow-lg"
        >
      ...
      >
            <div className="flex h-full flex-col overflow-hidden rounded-[30px] bg-[#F8FAF9] shadow-2xl">
              <div className="flex items-center justify-between gap-3 px-4 py-3 bg-white/95 border-b border-[#D1FAE5]">
                <div className="flex items-center gap-3">
                  <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-[#F0FDF4] text-cyan-300 ring-1 ring-cyan-300/20">
                    <Bot className="h-5 w-5" />
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-[#1E293B]">Gemini AI</p>
                    <p className="text-[11px] text-[#475569]">{roleLabels[role]} · {roleHints[role]}</p>
                  </div>
                </div>
                <Button variant="ghost" size="icon" onClick={() => setOpen(false)} className="text-[#14532D] hover:bg-white/10">
                  <X className="h-4 w-4" />
                </Button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
                {messages.map((msg, index) => (
                  ...
                  >
                    <div className={`flex max-w-[82%] gap-3 ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
                      <div className={`flex h-8 w-8 items-center justify-center rounded-full ${msg.role === "user" ? "bg-[#10B981]/10 text-blue-200" : "bg-[#10B981] text-[#1E293B]"}`}>
                        {msg.role === "user" ? <User className="h-4 w-4" /> : <Bot className="h-4 w-4" />}
                      </div>
                      <div className={`rounded-3xl px-3 py-2 text-sm leading-5 ${msg.role === "user" ? "bg-[#F8FAF9] text-[#1E293B]" : "bg-[#F0FDF4] text-[#1E293B]"}`}>
                        {msg.role === "assistant" ? (
                          <div className="prose prose-sm max-w-none text-[#1E293B] [&>p]:m-0 [&>ul]:my-1 [&>li]:mt-1">
                ...
                {isLoading && (
                  <div className="flex items-center gap-2 text-[#475569]">
                    <div className="flex h-8 w-8 items-center justify-center rounded-full bg-[#F0FDF4] text-cyan-300">
              ...
              <div className="space-y-3 border-t border-[#D1FAE5] bg-white/95 px-4 py-3">
                <div className="flex flex-wrap gap-2">{quickPromptButtons}</div>
                <div className="flex items-center gap-2">
                  <Button
                    variant="ghost"
                    size="icon"
                    className="text-[#14532D] hover:bg-white/10"
                  ...
                  <Input
                    ...
                    className="h-11 rounded-2xl bg-[#F8FAF9] text-[#1E293B] placeholder:text-[#475569]"
                    disabled={isLoading}
                  />
                  <Button
                    size="icon"
                    className="h-11 w-11 rounded-2xl bg-[#10B981] text-[#1E293B] hover:bg-cyan-400"
