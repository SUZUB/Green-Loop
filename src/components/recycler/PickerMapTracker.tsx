...
export function PickerMapTracker({ role = "picker", title, onHeatmapEvent }: PickerMapTrackerProps) {
  ...
  return (
    <div className="grid gap-6">
      <div className="h-[34rem] overflow-hidden rounded-[36px] border border-white/10 bg-slate-950/80 shadow-2xl">
        <MapContainer
        ...
        {highlightedReportId && (
          <div className="rounded-[28px] border border-cyan-400/20 bg-cyan-500/10 p-4 text-slate-100 mt-4">
      ...
      <div className="grid gap-6">
        <div className="rounded-[36px] border border-white/10 bg-slate-950/80 p-6 shadow-2xl">
          <div className="space-y-5">
            <div className="space-y-3">
              <p className="text-xs uppercase tracking-[0.24em] text-slate-400">Challenge Master</p>
              <h2 className="text-2xl font-semibold text-white">{titleText}</h2>
              <p className="text-sm text-slate-400">{subtitle}</p>
            </div>

            <div className="grid gap-3 sm:grid-cols-2">
              <div className="rounded-[28px] bg-slate-900/70 p-4">
                <p className="text-xs uppercase tracking-[0.24em] text-slate-400">Active Missions</p>
                <p className="mt-3 text-3xl font-semibold text-white">{challenges.length}</p>
                <p className="mt-2 text-xs text-slate-500">Nearby within the map bounds.</p>
              </div>
              <div className="rounded-[28px] bg-slate-900/70 p-4">
                <div className="flex items-center justify-between gap-2">
                  <p className="text-xs uppercase tracking-[0.24em] text-slate-400">Live Heat</p>
                  <Badge className="bg-cyan-100 text-cyan-800">{averageIntensity.toFixed(2)}</Badge>
                </div>
                <p className="mt-3 text-sm text-slate-300">Avg zone intensity across tracked points.</p>
              </div>
            </div>
        ...
        <aside className="flex flex-col gap-6">
          <div className="rounded-[36px] border border-white/10 bg-slate-950/80 p-6 shadow-2xl">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-xs uppercase tracking-[0.24em] text-slate-400">Mission Briefing</p>
                <h3 className="mt-2 text-lg font-semibold text-white">{selectedChallenge ? selectedChallenge.title : "Select a challenge"}</h3>
              </div>
              <Badge className="bg-white/10 text-slate-200">{selectedChallenge ? selectedChallenge.badgeLabel : "Ready"}</Badge>
            </div>
            <p className="mt-4 text-sm text-slate-400">{selectedChallenge ? selectedChallenge.description : "Choose a challenge card to expand the mission plan and join live."}</p>
          </div>

          <div className="flex-1 min-h-0 overflow-hidden rounded-[36px] border border-white/10 bg-slate-950/75 shadow-2xl">
            <AnimatePresence mode="wait">
              {detailsOpen && selectedChallenge ? (
                ...
                <motion.div
                  ...
                  exit={{ opacity: 0, y: 20 }}
                  className="flex h-full flex-col items-center justify-center gap-4 p-8 text-center text-slate-400"
                >
                  <div className="flex h-16 w-16 items-center justify-center rounded-full bg-slate-900/90 text-slate-300">
                    <span>👆</span>
                  </div>
