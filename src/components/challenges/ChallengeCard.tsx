...
export function ChallengeCard({ challenge, selected = false, onSelect, onJoin }: ChallengeCardProps) {
  ...
  return (
    <motion.button
      ...
      className={`w-full rounded-[26px] border p-5 text-left transition-all ${selected ? "border-cyan-400 bg-slate-900" : "border-white/10 bg-slate-950"}`}
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <Badge className={challenge.badgeClass}>{challenge.badgeLabel}</Badge>
            <span className="text-[11px] text-slate-400">{challenge.locationName}</span>
          </div>
        ...
        <div className="flex flex-col items-end gap-2 text-right text-xs text-slate-400">
          <span className="inline-flex items-center gap-1 text-slate-300"><MapPin className="h-3.5 w-3.5" /> {challenge.highlight}</span>
          <span className="inline-flex items-center gap-1 text-slate-300"><Clock3 className="h-3.5 w-3.5" /> {challenge.meetupTime}</span>
          <span className="inline-flex items-center gap-1 text-slate-300"><ShieldCheck className="h-3.5 w-3.5" /> {challenge.status}</span>
        </div>
      </div>

      <p className="mt-4 text-sm leading-6 text-slate-400">{challenge.description}</p>
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2">
        ...
        <div className="rounded-3xl bg-slate-900/80 p-4">
          <p className="text-[11px] uppercase tracking-[0.24em] text-slate-500">Reward</p>
