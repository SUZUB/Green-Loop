...
import {
  Home, Zap, User, Wallet, Trophy, Leaf, Award, Share2, BookOpen, Users, MapPin,
} from "lucide-react";
...
const recyclerToolsItems: NavigationItem[] = [
  ...
  { key: "nearbyRVMs",  icon: MapPin,  label: "Nearby RVMs", path: "/recycler/nearby-rvms" },
...
export function RecyclerSidebar({
      ...
      >
        ...
        >
          <Icon 
            ...
            className={`shrink-0 transition-colors duration-200 ${
              active ? "text-[#14532D]" : "text-[#475569] group-hover:text-[#14532D]"
            }`}
          />
        </motion.div>
        ...
        {sidebarOpen && (
          <span className={`text-sm font-semibold transition-colors duration-200 ${
            active ? "text-[#14532D]" : "group-hover:text-[#14532D]"
          }`}>
            {item.label}
          </span>
        )}
  ...
  const renderQuickActionTile = (item: QuickActionItem) => {
    ...
    return (
      <motion.button
        ...
        transition={{ type: "spring", stiffness: 400, damping: 10 }}
        className="flex flex-col items-center justify-center gap-2 p-3 rounded-xl border border-[#D1FAE5] bg-[#DCFCE7] backdrop-blur-md transition-all duration-150 group hover:bg-[#A7F3D0] hover:border-[#10B981] hover:shadow-md"
      >
        <Icon
          size={20}
          className="text-[#14532D] transition-transform duration-200 group-hover:scale-110"
        />
        <span className="text-[11px] font-semibold text-[#14532D] text-center leading-tight">
          {item.label}
        </span>
      </motion.button>
    );
  ...
  return (
    <aside
      className={`h-full min-h-screen ${sidebarOpen ? "p-5 space-y-6" : "p-3 space-y-4"} overflow-y-auto scrollbar-hide flex flex-col border-r transition-all duration-200`}
      style={{ background: "#F9FAFB", borderColor: "#DCFCE7" }}
    >
      {/* GREEN LOOP brand */}
      <motion.div
      ...
      >
        ...
        {sidebarOpen && (
          <div>
            <p className="text-sm font-bold text-[#1E293B] tracking-wider">GREEN LOOP</p>
            <p className="text-xs text-[#475569]">Recycler</p>
          </div>
        )}
      </motion.div>
      ...
      <nav className="space-y-3">
        {sidebarOpen && <h3 className="text-[#475569] text-sm font-semibold px-2 mb-3">Navigation</h3>}
      ...
      <nav className="space-y-3">
        {sidebarOpen && (
          <h3 className="text-[#475569] uppercase tracking-widest text-[10px] font-semibold px-4 mb-3">
            Recycler Tools
          </h3>
        )}
        <div className="space-y-2">
      ...
      {sidebarOpen && (
      <div className="space-y-3 pt-4 border-t border-[#D1FAE5]">
        <h3 className="text-[#475569] text-sm font-semibold px-2 mb-3">Live Metrics</h3>
        <div className="space-y-3">
          {/* Global Impact */}
          <motion.div
          ...
          >
            <p className="text-xs font-semibold text-[#475569] mb-1">Global Impact</p>
            <motion.div
              layout
          ...
          <motion.div
            ...
            className="rounded-xl border border-[#D1FAE5] bg-[#F0FDF4] p-3"
          >
            <p className="text-xs font-semibold text-[#475569] mb-1">Live Pickers</p>
            <motion.div
              layout
              className="flex items-baseline gap-1"
            >
              ...
              <span className="text-xs text-[#475569]">active</span>
            </motion.div>
          </motion.div>
          ...
          <motion.div
            whileHover={{ scale: 1.02 }}
            className="relative rounded-xl border border-[#D1FAE5] bg-[#F0FDF4] p-3 overflow-hidden"
          >
            <PulseEffect trigger={impactPulseKey} color="rgba(76, 175, 125, 0.4)" intensity={1.2} />
            <p className="text-xs font-semibold text-[#475569] mb-1">My Impact</p>
            <motion.div
              layout
              className="flex items-baseline gap-1"
            >
      ...
      {/* Profile Section */}
      <div className="pt-4 border-t border-[#D1FAE5] mt-auto space-y-2">
        <motion.button
          whileHover={{ x: 4 }}
        ...
        >
          <User 
            ...
            strokeWidth={1.5}
            className={`shrink-0 transition-colors duration-200 ${
              isActive("/recycler/profile") ? "text-[#14532D]" : "text-[#475569] group-hover:text-[#14532D]"
            }`}
          />
        </motion.div>
        ...
        {sidebarOpen && (
          <span className={`text-sm font-semibold transition-colors duration-200 ${
            isActive("/recycler/profile") ? "text-[#14532D]" : "group-hover:text-[#14532D]"
          }`}>
            Profile
          </span>
        )}
        ...
        {sidebarOpen && <button
          ...
          className="w-full flex items-center justify-center gap-2 text-[#475569] hover:text-[#10B981] py-3"
        >
